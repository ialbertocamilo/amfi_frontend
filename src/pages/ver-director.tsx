        ...
        {director && (
            <div className="bg-white p-6 rounded-lg shadow-lg transition-transform transform hover:scale-105">
                ...
                <p className="text-gray-600"><strong>Fecha de
                    Nacimiento:</strong> {new Date(director.birthDate).toLocaleDateString()}</p>
                <p className="text-gray-600"><strong>Residente
                    Mexicano:</strong> {director.isMexicanResident ? 'Sí' : 'No'}</p>
                <p className="text-gray-600"><strong>Año de Inicio de
                    Experiencia:</strong> {director.startedExperienceYear}</p>
                <p className="text-gray-600"><strong>Disponible:</strong> {director.isAvailable ? 'Sí' : 'No'}</p>
                <p className="text-gray-600">
                    <strong>Representación:</strong> {ProjectMapper.mapRepresentationType(director.representation)}</p>
            </div>
        )}
    </div>
</div>
