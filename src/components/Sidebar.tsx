...
const Sidebar: React.FC = () => {
  ...
  const [companyType, setCompanyType] = useState(CompanyType.ProductionStudio)
  useEffect(() => {
  ...
  const getLinksForRole = (role: string | null, companyType: CompanyType | null) => {
    if (!role) return [];
    const roleLinks = [...links.common, ...links[role]];
    if (companyType === CompanyType.ProductionStudio) {
      roleLinks.push({ href: '/directores', icon: 'IconDirectores.png', label: 'Directores' });
    }
    return roleLinks;
  };
